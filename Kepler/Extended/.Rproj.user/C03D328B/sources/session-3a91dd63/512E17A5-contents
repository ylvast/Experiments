...
experiment_func <- function(path,P,ninit,nfinal,params,probs,transforms,ex,chain_number,seed,model=FALSE){
  set.seed(seed)
  if (model==FALSE){
...
for (ex in c(7:12)) {
  ...
  for (run in c(1:30)) {
    ...
    parallel_runs <- mclapply(seq_len(experiment_config$B[2]), function (i) experiment_func(dir_path_seed,P,ninit,nfinal,params,probs,transforms,ex,i,run), mc.cores=experiment_config$B[2])
  }
}
